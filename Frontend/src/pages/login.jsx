...
export default function Login() {
    ...
    <form class="flex flex-col w-fit h-fit items-center justify-center" method="POST">
      ...
      <label class="self-start w-10/12 mt-5 mb-1 pl-2 mx-auto" for="password">Password</label>
      <div
        class="border-2 border-slate-400  w-10/12 h-12 rounded-md flex items-center justify-center outline-none focus-within:outline-slate-500 focus-within:border-0 text-md text-slate-900 placeholder-slate-600 pr-5">
        <input
      ...
      <div class="flex justify-between w-10/12 mt-5">
        <div class="text-black font-semibold flex items-center gap-2">
          <input class="h-4 w-4" type="checkbox" name="remember-me" id="remember-me"/>
          <span>Remember Me</span>
        </div>
        <div>
          <a class="text-slate-600 hover:text-slate-800" href="#">Forgot Password?</a>
        </div>
      </div>
      <button class="bg-blue-600 mt-5 w-10/12 h-12 rounded-md flex items-center justify-center text-white font-semibold"
        type="submit">
        Login
      </button>
      ...
      <div class="flex gap-2 mt-8"><span class="font-semibold text-slate-600 ">Don't have an account?</span> <a
          class="text-blue-700 font-semibold hover:underline" href="/signup">Sign Up</a></div>
    </form>
  </div>
